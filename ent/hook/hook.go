...
// The GuessFunc type is an adapter to allow the use of ordinary
// function as Guess mutator.
type GuessFunc func(context.Context, *ent.GuessMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f GuessFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.GuessMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.GuessMutation", m)
	}
	return f(ctx, mv)
}
